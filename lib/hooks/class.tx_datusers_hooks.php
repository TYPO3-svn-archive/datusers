...
/**
 * 'Hooks' for the 'datusers' extension. Hook methods for foreign extensions liek felogin.
 *
 ...
 */
  public function felogin_postProcContent( $params, &$pObj )
  {
    ...
    $GLOBALS['TSFE']->fe_user->setKey( 'user', 'datusers', $sessData );
    $GLOBALS["TSFE"]->storeSessionData();
      // Move session date from a non logged in user to a logged in user
      
//    if ( $this->drsHooks )
//    {
//      $prompt = print_r( $GLOBALS['TSFE']->fe_user->getKey( 'ses', 'datusers' ), true );
//      t3lib_div::devlog( '[INFO/HOOKS] ses: ' . $prompt, $this->extKey, 3 );
//      $prompt = print_r( $GLOBALS['TSFE']->fe_user->getKey( 'user', 'datusers' ), true );
...
//    }
    ...
    $this->pObj = $pObj;
      // Class var $conf
    $this->conf = $pObj->conf['extensions.']['datusers.'];

      // Current content
    $content = $params['content'];
      // Form login of the felogin extension with replaced datusers markers
...
//var_dump( __METHOD__, __LINE__, t3lib_div::_GET( ), t3lib_div::_POST( ), $GLOBALS['TSFE']->fe_user->getKey( 'ses', 'datusers' ) );
    ...
    switch( true )
    {
      case( $logintype == 'logout' ):
        ...
        if ( $this->drsHooks )
        {
          $prompt = 'Session data datusers are removed.';
          t3lib_div::devlog( '[INFO/HOOKS] ' . $prompt, $this->extKey, 0 );
        }
        return;
        break;
    ...
    if ( $this->drsHooks )
    {
      $prompt = 'Session data datusers aren\'t removed.';
      t3lib_div::devlog( '[INFO/HOOKS] ' . $prompt, $this->extKey, 0 );
    }
      // DRS
  }
...
//    $sessData['datusers_clientno'] = $sessData['datusers_clientno'];
    ...
    {
      $prompt = 'Session data datusers are updated.';
      t3lib_div::devlog( '[INFO/HOOKS] ' . $prompt, $this->extKey, 0 );
    }
      // DRS
  }
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/browser/pi1/class.tx_datusers_hooks.php'])
{
  include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/browser/pi1/class.tx_datusers_hooks.php']);
}
