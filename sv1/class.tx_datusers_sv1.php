 ...
 *
 *
 *   91: class tx_datusers_sv1 extends tx_sv_authbase
 *
 *              SECTION: Main
 ...
 *
 *              SECTION: DAT
 *  803:     private function datGroupsToTYPO3Groups( $datGroups )
 *
 *              SECTION: fe_users
 ...
 * 1470:     private function soapDatAuthenticate( )
 * 1524:     private function soapDatAuthenticateTestuser( )
...
/**
 * Service "Authorise a DAT user" for the "datusers" extension.
 *
 ...
 * @since       0.0.3
 */
class tx_datusers_sv1 extends tx_sv_authbase
{
 /**
  * Class name
  *
  * @var string
  */
  protected $prefixId  = 'tx_datusers_sv1';
  ...
  * @var string
  */
  private $scriptRelPath  = 'sv1/class.tx_datusers_sv1.php';
 ...
 /**
  * SOAP groups of the current user
  *
  * @var array
  */
 ...
 /**
  * DAT groups of the testuser
  *
  * @var array
  */
  private $testuserDatGroups = null;
  ...
  public function getUser( )
  {
    ...
    switch( true )
    {
      case( $logintype == 'login' ):
        ...
        $sessData['datusers_clientno']  = t3lib_div::_GP( 'datusers_clientno' );
        break;
      default:
        $sessData = $GLOBALS['TSFE']->fe_user->getKey( 'ses', 'datusers' );
        break;
    }
      // Session data - step 1 from 3
...
//var_dump( __METHOD__, __LINE__, $this->loginData['uname'], $sessData['username'], $GLOBALS['TSFE']->fe_user->user['uid'], $feuser_record );
    ...
    $GLOBALS['TSFE']->fe_user->setKey( 'ses', 'datusers', $sessData );
    ...
    {
      $sessData = $GLOBALS['TSFE']->fe_user->getKey( 'ses', 'datusers' );
      $prompt   = 'datusers: ' .  t3lib_div::arrayToLogString
                                  (
                                    $sessData,
                                    array( ),
                                    100
 ...
 */
  ...
  {
      // Init admin customer number
    $this->adminDatCustomerno = $this->arr_extConf['adminDatCustomerno'];
      // RETURN: SOAP mode is disabeld
    if( empty ( $this->adminDatCustomerno ) )
    {
      if ( $this->writeDevLog )
      {
        $prompt = 'Admin for the SOAP server hasn\'t any customer number.';
    ...
    $this->adminDatLogin = $this->arr_extConf['adminDatLogin'];
      // RETURN: SOAP mode is disabeld
    if( empty ( $this->adminDatLogin ) )
    {
      if ( $this->writeDevLog )
      {
      ...
      // Init admin password
    $this->adminDatPassword = $this->arr_extConf['adminDatPassword'];
      // RETURN: SOAP mode is disabeld
    if( empty ( $this->adminDatPassword ) )
    {
      if ( $this->writeDevLog )
      {
    ...
    $status_message = '
      ...
      <ul>
        <li>
          ...
          The SOAP test mode is managed by the TYPO3 extension datusers.
        </li>
      </ul> ';

      // Set session data: status message
    $sessData = $GLOBALS['TSFE']->fe_user->getKey( 'ses', 'datusers' );
    $sessData['datusers_soaptest_message'] = $status_message;
    $GLOBALS['TSFE']->fe_user->setKey( 'ses', 'datusers', $sessData );
      // Set session data: status message
  }
...
//          </p>';
//        $sessData = $GLOBALS['TSFE']->fe_user->getKey( 'ses', 'datusers' );
//        $sessData['datusers_status_message'] = $status_message;
//        $GLOBALS['TSFE']->fe_user->setKey( 'ses', 'datusers', $sessData );
        return false;
        break;
      default:
  ...
  */
  private function requiredClientNo( )
  {
    ...
    switch( true )
    {
      case( isset ( $_POST['datusers_clientno'] ) ):
          // follow the workflow
        break;
      case( isset ( $_GET['datusers_clientno'] ) ):
          // follow the workflow
        break;
      default:
    ...
    $clientNo = t3lib_div::_GP( 'datusers_clientno' );
    if( empty ( $clientNo ) )
    {
      if ( $this->writeDevLog )
      {
...
//      }
 ...
 /**
  * datGroupsToTYPO3Groups( ):
  *
  * @param    array        $datGroups: DAT user groups
  * @return    boolean        true: password is empty; false: password isn't empty
  * @version  0.1.1
  * @since    0.1.0
  */
  private function datGroupsToTYPO3Groups( $datGroups )
  {
    $arrTYPO3Groups = array( );
    $csvTYPO3Groups = null;

      // RETURN null: DAT groups are empty
    if( empty( $datGroups ) )
    {
        // DRS prompt
      if ( $this->writeDevLog )
      {
        $prompt = 'User isn\'t attributed to any DAT user group.';
        t3lib_div::devlog( '[WARN/SERVICES] ' . $prompt, $this->extKey, 3 );
      }
        // DRS prompt
      return null;
    ...
    $pid                = ( int ) $this->arr_extConf['fe_usersPid'];
      // andWhere
    $andWhereDatGroups  = implode( "' OR datgroup LIKE '", $datGroups );
    $andWhereDatGroups  = " AND (datgroup LIKE '" . $andWhereDatGroups . "')";

      // Query
    $select_fields  = 'fe_groups';
    $from_table     = 'tx_datusers_groups';
    $where_clause   = "pid = " . $pid . $andWhereDatGroups;
    $groupBy        = '';
    $orderBy        = '';
    $limit          = '';
      // Query
...
//    var_dump( __METHOD__, __LINE__, $query );
    ...
    if( empty( $row ) )
    {
      ...
      if ( $this->writeDevLog )
      {
        ...
        // Set session data: status message
      $sessData = $GLOBALS['TSFE']->fe_user->getKey( 'ses', 'datusers' );
      $status_message = $sessData['datusers_status_message'];
  ...
  */
  private function feuserSetUsergroupBySoapuser( )
  {
    $csvTYPO3Groups = null;
      ...
      // Convert DAT groups to TYPO3 groups
    $csvTYPO3Groups = $this->datGroupsToTYPO3Groups( $this->soapGroups );
  ...
  */
  private function feuserSetUsergroupByTestuser( )
  {
    $csvTYPO3Groups    = null;
      ...
      // Convert DAT groups to TYPO3 groups
    $csvTYPO3Groups = $this->datGroupsToTYPO3Groups( $this->testuserDatGroups );
    ...
    if( empty ( $usergroup ) )
    {
      ...
      $status_message = $status_message . '
                        ...
                        <ul>
                          ...
                          <li>
                            User won\'t authenticated by TYPO3 extension datusers.
                          </li>
                        </ul>
                        <h2>
                          What can you do?
                        ...
                        <ul>
                          <li>
                            Please report this problem to the DAT group administzrator.
                          </li>
                        </ul>';
      $sessData['datusers_status_message'] = $status_message;
      $GLOBALS['TSFE']->fe_user->setKey( 'ses', 'datusers', $sessData );
...
//    $username = $this->loginData['uname'];
    ...
    (
      ...
      'address'       => $user->street . ' ' . $user->streetNumber,
      'zip'           => $user->zip,
      'city'          => $user->city,
...
$this->feuserRecord = $row;
 ...
 /**
  * soapDatAuthenticate( ): Authenticates the user by the SOAP server. If authentication is
  ...
  * @since    0.1.0
  */
  private function soapDatAuthenticate( )
  {

      // RETURN: SOAP test is enabled
    if( $this->soaptest )
    {
      return $this->soapDatAuthenticateTestuser( );
    }
      // RETURN: SOAP test is enabled

      // Current login data
    $this->datCustomerno  = t3lib_div::_GP( 'datusers_clientno' );
    $this->datLogin       = $this->loginData['uname'];
    $this->datPassword    = $this->loginData['uident'];
      // Current login data
...
//    $this->soapErrorAddTestPrompt( );
 ...
 /**
  * soapDatAuthenticateTestuser( ):
  *
  * @return    boolean        true: authentication without any error; false: authentication with an error
  * @version  0.1.0
  * @since    0.1.0
  */
  private function soapDatAuthenticateTestuser( )
  {
    $this->datCustomerno  = t3lib_div::_GP( 'datusers_clientno' );
    $this->datLogin       = $this->loginData['uname'];
    $this->datPassword    = $this->loginData['uident'];
    ...
    switch( true )
    {
      case( $this->datCustomerno  != $this->arr_extConf['testuserDatCustomerno']  ):
      case( $this->datLogin       != $this->arr_extConf['testuserDatLogin']       ):
      case( $this->datPassword    != $this->arr_extConf['testuserDatPassword']    ):
      ...
      // Get DAT groups
    $csvDatGroups = $this->arr_extConf['testuserDatGroups'];
    $arrDatGroups = explode( ',', $csvDatGroups );

    foreach( ( array ) $arrDatGroups as $key => $value )
    {
      $arrDatGroups[$key] = trim( $value );
      if( empty( $arrDatGroups[$key] ) )
      {
        unset( $arrDatGroups[$key] );
      }
    }
      // Get DAT groups

      // RETURN: Authentification failed - step 2 from 2: no groups
    if( empty( $arrDatGroups ) )
    {
      if ( $this->writeDevLog )
      {
      ...
      $status_message = $status_message . '
                        ...
                        <ul>
                          <li>
                            Please take care of proper DAT groups.
                          </li>
                          <li>
                            You maintain the DAT groups in the estension manager of the TYPO3
                            extension datusers.
                          </li>
                        </ul>';
      $sessData['datusers_status_message'] = $status_message;
      ...
      // Set global $testuserDatGroups
    $this->testuserDatGroups = $arrDatGroups;

      // DRS
    if ( $this->writeDevLog )
    {
      $prompt = 'Authentification of the testuser is successfull: access data are proper, user
                is attributed to some DAT groups.';
      t3lib_div::devlog( '[OK/SERVICES] ' . $prompt, $this->extKey, -1 );
    }
      // DRS
  ...
  private function soapGetUser( )
  {
    ...
    try
    {
      $user = $this->soapclient->getUser
              (
                $this->adminDatCustomerno,
                $this->adminDatLogin,
                $this->adminDatPassword,
                $this->datCustomerno,
                $this->datLogin
              );
      if( empty ( $user ) )
  ...
  *               See sesssion array datusers_status_message.
  *
  * @param    [type]        $$method: ...
  * @param    [type]        $line: ...
  * @param    [type]        $fault: ...
    ...
    switch( $fault->faultcode )
    {
      case( 'HTTP' ):
        ...
        $status_message = $status_message . PHP_EOL . '
                          ...
                          <ul>
                            <li>
                              <strong>If your are a DAT customer,</strong> please send an e-mail to the
                            ...
                            <li>
                              ...
                              The data exchange with the SOAP server is managed by the TYPO3 extension datusers.
                            </li>
                          </ul> ';
        break;
      case( 'WSDL' ):
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/datusers/sv1/class.tx_datusers_sv1.php'])
{
  include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/datusers/sv1/class.tx_datusers_sv1.php']);
}
